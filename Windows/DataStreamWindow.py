...
class DataStreamWindow:
    def __init__(self, port, config_file='configJSON.json', test_mode=False):
        ...
        # Value ranges used to position gauge needles
        speed_max = 160 if self.speed_unit == "MPH" else 260
        self.gauge_ranges = {
            'RPM': (0, 8000),
            'Speed': (0, speed_max),
            'TPS': (0, 100),
            'Temp': (-40, 250 if self.temp_unit == "F" else 120),
            'Timing': (-10, 50),
            'Battery': (8, 16),
            'AAC': (0, 100),
            'Injector': (0, 20)
    ...
    def _create_gauge(self, row, col, sensor_key, frame_width, frame_height):
        ...
        gauge.create_text(center_x, center_y - (radius * 0.6), text=str(title), font=("Arial", 12, "bold"))
